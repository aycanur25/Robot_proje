...
from Sac_Network.utilities.Network import Network
from Sac_Network.utilities.ReplayBuffer import ReplayBuffer
...
class SACAgent:
    ...
    def get_action_nondeterministically(self, state):
        ...
        # Eğer action_probabilities bir tuple ise, tensor'u alıyoruz
        if isinstance(action_probabilities, tuple):
            action_probabilities = action_probabilities[0]  # İlk öğeyi alıyoruz, gerekirse bu değiştirilebilir
    
    # Tensor olup olmadığını kontrol etme
        if isinstance(action_probabilities, torch.Tensor):
            if action_probabilities.requires_grad:
                action_probabilities = action_probabilities.detach().numpy()
            else:
                action_probabilities = action_probabilities.numpy()

        # action_probabilities'ı 1 boyutlu hale getirme
        action_probabilities = np.squeeze(action_probabilities)

    # Eğer hala çok boyutlu ise, onu düzleştiriyoruz
        if action_probabilities.ndim > 1:
            action_probabilities = action_probabilities.flatten()

    # Aksiyonları seçim yaparak alıyoruz
    ...
    def critic_loss(self, states_tensor, actions_tensor, rewards_tensor, next_states_tensor, done_tensor):
        ...
        #print(type(actions_tensor.unsqueeze(-1)))
        actions_tensor = actions_tensor.type(torch.int64)
    ...
    # Eğer state bir tuple ise, ilk öğeyi alın (örneğin (state, info) şeklinde olabilir)
        if isinstance(state, tuple):
            state = state[0]  # Sadece state kısmını al

    # State'in numpy array veya liste olduğundan emin olun
        if isinstance(state, np.ndarray):
            # Eğer state 1D değilse, flatten edip tek boyutlu hale getirelim
            state = state.flatten()  # Tek boyutlu hale getirelim
            state_tensor = torch.tensor(state, dtype=torch.float32).unsqueeze(0)  # 2D tensor'a dönüştür
        elif isinstance(state, list):
            state = np.array(state).flatten()  # Listeyi numpy array'e çevirip düzleştir
            state_tensor = torch.tensor(state, dtype=torch.float32).unsqueeze(0)  # 2D tensor'a dönüştür
        else:
        # Eğer state başka bir türdeyse, hata veririz
            raise ValueError("State, numpy array veya liste olmalı.")

    # Aksiyon olasılıklarını almak için forward işlemi yapıyoruz
        action_probabilities = self.actor_local.forward(state_tensor)
        z = action_probabilities == 0.0
        z = z.float() * 1e-8  # Log 0'dan kaçınmak için küçük bir değer ekle
        log_action_probabilities = torch.log(action_probabilities + z)
        return action_probabilities, log_action_probabilities
